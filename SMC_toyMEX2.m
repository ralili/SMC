...
function [loglikelihood,Theta,ev,gF,H] = SMC_toyMEX2(hyperparameters)%evidence, gradient of the evidence, hessian of the evidence
%% Setup sampler
%%
...
for b = 2:blength   % b = n (from paper)
    ...
    parfor m = 1:M%%STUFF IS FAILING BECAUSE THE IF STATEMENT DOES NOT WORK CORRECTLY. WHAT IS HAPPENING?
        ...
        while (acc<=burn)                       %MCMC
            ...
            propparams = exp(prop);
            P_data=feval('twoPaths2',timeVector,x0,[propparams(end-2:end)',path1,path2,propparams(1:2)']);
