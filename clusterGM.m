function [S,NlogL,optimInfo]...
    =clusterGM(X,k,start,reps, CovType,SharedCov, RegV, options)
...
%
%            'rndsample'   Select K observations from X at random as the
...
%                  requires the 'rndsample' start method.
%
...
%   $Revision: 1.1.6.3 $  $Date: 2008/06/20 09:05:40 $$
...
elseif ~strcmp(start,'rndSample')
    %error('stats:gmdistribution:UnknownStart', ...
        %'The ''Start'' parameter value must be ''rndsample'', a struct or a vector of integers.');
end
...
for t = 1:reps
    ...
    try
        ...
        if reps == 1 || ~isequal(ME.identifier,'stats:gmdistribution:IllCondCov')
            ...
            if illCondCnt == reps
                throwAsCaller(MException('stats:gmdistribution:IllCondCovAllReps', ...
...
for iter = 1:options.MaxIter;
    ...
    try
        ...
        if ~isequal(ME.identifier,'stats:gmdistribution:wdensity:IllCondCov')
            ...
            throwAsCaller(MException('stats:gmdistribution:IllCondCov', ...
    ...
    if SharedCov %common covariance
        ...
        for j = 1:k
            S.mu(j,:) = post(:,j)' * X / S.PComponents(j);
            %Xcentered = bsxfun(@minus, X, S.mu(j,:));
            Xcentered = X-S.mu(j*ones(n,1),:);
            if CovType == 2
                %Xcentered = bsxfun(@times,sqrt(post(:,j)),Xcentered);
                Xcentered = sqrt(post(:,j*ones(d,1))).*Xcentered;
