...
function [loglikelihood,Theta,ev,gF,H] = SMC_toyMEXadaptive(hyperparameters,Y_data)%evidence, gradient of the evidence, hessian of the evidence
if nargin < 2
    Y_data=0;
end
...
while beta_schedule(end)<1
    ...
    parfor m = 1:M%%STUFF IS FAILING BECAUSE THE IF STATEMENT DOES NOT WORK CORRECTLY. WHAT IS HAPPENING?
        ...
        while (acc<=burn)                       %MCMC
            ...
            propparams = exp(prop);
            P_data=feval('twoPaths2',timeVector,x0,[initial_points(fixed_parameters,m)',path1,path2,propparams']);%%%%Fixed parameters as initial values
    ...
    betastep = c;
    beta_schedule = [beta_schedule min(beta_schedule(end)+betastep,1)]%%%%Why do we sum up the beta_schedule?????
    %
...
ev = -marg_likelihood
gF = ev*[(mean(log(Theta{end}(variable_parameters,:)'))-mean(log(initial_points(variable_parameters,:)'))),(mean(log(Theta{end}(variable_parameters,:)').^2)-mean(log(initial_points(variable_parameters,:)').^2))];
