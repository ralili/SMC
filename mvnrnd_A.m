function [r,T] = mvnrnd_A(mu,sigma,cases)
...
%   $Revision: 2.13.4.8 $  $Date: 2008/02/29 13:12:10 $
...
%               'equal to the number of columns in MU.']);
%     end
...
%         if any(sigma<0)
%             error('stats:mvnrnd:BadDiagSigma', ...
%                 'All elements of diagonal SIGMA must be non-negative.');
%         end
%         t = sqrt(sigma);
%         if nargout>1
%             T = diag(t);
...
%     else
        ...
        T = sigma; % argument sigma is actually the cholesky factor of sigma 
%         [T,err] = cholcov_A(sigma);
%         if err ~= 0
%             error('stats:mvnrnd:BadCovariance',...
...
%         error('stats:mvnrnd:BadCovariance',...
%               'Each page of SIGMA must be a vector or a square matrix.');
%     elseif (sz(1) ~= d) || (sz(2) ~= d) % Sigma is 3-D
%         error('stats:mvnrnd:InputSizeMismatch',...
%               ['Each page of SIGMA must be a square matrix with size '...
%               'equal to the number of columns in MU, or a row vector with '...
%               'length equal to the number of columns in MU.']);
%     elseif sz(3) ~= n
%         error('stats:mvnrnd:InputSizeMismatch','SIGMA must have CASES number of pages.');
%     end
%   
%     if nargin < 4
%         if nargout > 1
...
%             for i = 1:n
%                 [R,err] = cholcov_A(sigma(:,:,i));
%                 if err ~= 0
%                     error('stats:mvnrnd:BadCovariance',...
%                         ['Each page of SIGMA must be a symmetric '...
