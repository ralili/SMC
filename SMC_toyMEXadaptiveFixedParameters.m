...
function [ev,gF,H] = SMC_toyMEXadaptiveFixedParameters(hyperparameters,Y_data)%evidence, gradient of the evidence, hessian of the evidence
if nargin < 2
    Y_data=0;
end
...
parfor m = 1:M
    P_data=feval('twoPaths2',timeVector,x0,[initial_points(fixed_parameters,m)',path1,path2,initial_points(variable_parameters,m)']);
...
while beta_schedule(end)<1
    ...
    parfor m = 1:M%%STUFF IS FAILING BECAUSE THE IF STATEMENT DOES NOT WORK CORRECTLY. WHAT IS HAPPENING?
        ...
        while (acc<=burn)                       %MCMC
            ...
            propparams = exp(prop);
            P_data=feval('twoPaths2',timeVector,x0,[initial_points(fixed_parameters,m)',path1,path2,propparams']);%fixed variables are set to the initial points.
...
ev = -marg_likelihood
gF = ev*[(mean(log(Theta{length(beta_schedule)-1}(variable_parameters,:)'))-mean(log(initial_points(variable_parameters,:)'))),(mean(log(Theta{length(beta_schedule)-1}(variable_parameters,:)').^2)-mean(log(initial_points(variable_parameters,:)').^2))];
Cprior = cov([log(initial_points(variable_parameters,:)'),log(initial_points(variable_parameters,:)').^2]);
Cpost = cov([log(Theta{length(beta_schedule)-1}(variable_parameters,:)'),log(Theta{length(beta_schedule)-1}(variable_parameters,:)').^2]);
H = ev*(Cpost-Cprior);
%%
