function [y,compIdx,T] = GMrnd_par(mu,cholsigma,p,n)
...
%      hist3(y,[25,25]);
%
%   See also mvnrnd_A, MVNPDF, MVNCDF, NORMRND.
...
y = zeros(n,d,superiorfloat(mu,cholsigma));

%if nargin < 5 || isempty(T)   % T not provided
    if ndims(cholsigma)==3 % multiple covariance
        %T = zeros(size(sigma),class(sigma));
        mbrs = {};
        Y = {};
        for i = 1:K
            mbrs{i} = find(compIdx == i);
            [Y{i},R] = mvnrnd_A(mu(i,:),cholsigma(:,:,i),length(mbrs{i}));
            %Rrows = size(R,1);
            %T(1:Rrows,:,i) = R;
        end
        for i = 1:K
        ...
        mbrs = find(compIdx == 1);
        [y(mbrs,:),T] = mvnrnd_A(mu(1,:),sigma,length(mbrs));
        for i = 2:K
            mbrs = find(compIdx == i);
            y(mbrs,:) = mvnrnd_A(mu(i,:),sigma,length(mbrs),T);
        end
    end
% else % T provided
...
%         for i = 1:K
%             mbrs = find(compIdx == i);
%             y(mbrs,:) = mvnrnd_A(mu(i,:),sigma(:,:,i),length(mbrs),T(:,:,i));
%         end
%     else % common covariance
%         for i = 1:K
%             mbrs = find(compIdx == i);
