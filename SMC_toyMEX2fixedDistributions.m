...
%I don't think this works. it's very similar to
%SMC_toyMEX2fixedParameters.m
...
function [ev,gF,H] = SMC_toyMEX2fixedDistributions(hyperparameters)%evidence, gradient of the evidence, hessian of the evidence
%% Setup sampler
%%
...
for b = 2:blength   % b = n (from paper)
    ...
    prev_theta = Theta{b-1}(1:parnumVar,:);
    ...
    if (resample)
        ...
        theta_res = [];
        for u = 1:M
            theta_res = [theta_res Theta{b-1}(1:parnumVar,u*ones(1,replicates(u)))];
            repl_ind = [repl_ind u*ones(1,replicates(u))];
        end
        rest = sum(replicates);
        for u = rest+1:M
            rndpoint = randsample(M,1,true,prob);
            theta_res = [theta_res Theta{b-1}(1:parnumVar,rndpoint)];
    ...
    parfor m = 1:M%%STUFF IS FAILING BECAUSE THE IF STATEMENT DOES NOT WORK CORRECTLY. WHAT IS HAPPENING?
        if (resample)
            start = prev_theta(:,m);             %if (resample), choose {X_{n-1}^i} according to weights {W_n^i}
            ...
            Eprev = LL(m);
            Pprev = max(sum(log(lognpdf(start,[meanlogpar',meanfixedpar'],[sigmalogpar',sigmafixedpar']))),-1e5);
        end
        x = zeros(parnumVar+parnumFixed,burn);
        x(:,1) = start;
        acc = 1;
        while (acc<=burn)                       %MCMC
...
%%
% for i=1:43
% hist(W(i,:),[0:max(W(i,:))/100:max(W(i,:))*1.3])
% i
% pause
% end
%%
